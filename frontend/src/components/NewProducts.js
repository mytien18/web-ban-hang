...
export default function NewProducts() {
    ...
    (async () => {
      ...
      try {
        const newest = await safeFetch(
          `${API_BASE}/api/v1/products-new?status=1&limit=10`
        );
  ...
  const Card = memo(({ prod }) => {
    const [adding, setAdding] = useState(false);
    const priceBuy = prod.price_buy ?? 0;
    const priceSale = prod.price_sale ?? null;
    const discountLabel = useMemo(() => calcDiscountLabel(priceBuy, priceSale), [priceBuy, priceSale]);
    const productHref = useMemo(() => `/product/${prod.slug || prod.id}`, [prod.slug, prod.id]);
    const isNew = prod.product_new === 1;
    ...
    }, [prod.id, prod.name, prod.thumbnail, priceSale, priceBuy]);
        ...
        <div className="relative w-full aspect-square overflow-hidden bg-gray-100">
          <Link href={productHref}>
            <Image
              ...
              loading="lazy"
              unoptimized={prod.thumbnail?.startsWith("http") && !prod.thumbnail.includes("127.0.0.1") && !prod.thumbnail.includes("localhost")}
            />
          </Link>
          ...
          {hasItems ? (
            ...
            >
              {items.slice(0, 10).map((p) => (
                <Card key={`newest-${p.id}`} prod={p} />
              ))}
            </div>
          ) : (
            <p className="text-center text-gray-500 py-12">Chưa có sản phẩm mới.</p>
          )}
        </div>
      </section>
    </>
