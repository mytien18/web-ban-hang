...
export default function PostsTrashPage() {
  ...
  const purgeRow = async (row) => {
    if (!confirm(`Xoá vĩnh viễn bài viết "${row.title}"? Hành động này không thể hoàn tác.`)) return;
    try {
  ...
  const bulkPurge = async () => {
    if (selectedCount === 0) return;
    if (!confirm(`Xoá vĩnh viễn ${selectedCount} bài viết đã chọn? Hành động không thể hoàn tác.`)) return;
    setBulkLoading(true);
    try {
  ...
  return (
    <>
      <Head>
        <title>Thùng rác — Bài viết | Admin</title>
      ...
      <main id="main-content" className="p-6 anim-fade-in" aria-busy={loading ? "true" : "false"}>
        ...
        <nav aria-label="Bạn đang ở đây" className="text-sm text-gray-500 mb-1">
          <ol className="flex items-center gap-1">
            ...
            <li><Link href="/admin/posts" className="hover:underline">Bài viết</Link></li>
            <li aria-hidden="true">/</li>
        ...
        <header className="mb-6 flex items-start md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold">🗑️ Thùng rác — Bài viết</h1>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/admin/posts" className="inline-flex">
              <Button variant="secondary" aria-label="Quay về danh sách bài viết">
                <ArrowLeft size={16} />
                Quay về bài viết
              </Button>
            </Link>
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50 text-left sticky top-0 z-10">
                  <tr>
                    ...
                    <th className="px-4 py-3 font-medium text-gray-600">Chủ đề</th>
                ...
                <tbody className="divide-y" aria-live="polite">
                  ...
                  {!loading && rows.map((row) => (
                    <tr key={row.id} className="hover:bg-gray-50/60 transition">
                      ...
                      <td className="px-4 py-3 text-gray-700">{row.post_type || "post"}</td>
