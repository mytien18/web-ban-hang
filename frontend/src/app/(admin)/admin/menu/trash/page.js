...
export default function MenuTrashPage() {
  ...
  const purgeRow = async (row) => {
    if (!confirm(`Xoá vĩnh viễn menu "${row.name}"? Hành động này không thể hoàn tác.`)) return;
    try {
  ...
  const bulkPurge = async () => {
    if (selectedCount === 0) return;
    if (!confirm(`Xoá vĩnh viễn ${selectedCount} menu đã chọn? Hành động không thể hoàn tác.`)) return;
    setBulkLoading(true);
    try {
  ...
  return (
    <>
      <Head>
        <title>Thùng rác — Menu | Admin</title>
      ...
      <main id="main-content" className="p-6 anim-fade-in" aria-busy={loading ? "true" : "false"}>
        <nav aria-label="Bạn đang ở đây" className="text-sm text-gray-500 mb-1">
          <ol className="flex items-center gap-1">
            ...
            <li><Link href="/admin/menu" className="hover:underline">Menu</Link></li>
            <li aria-hidden="true">/</li>
        ...
        <header className="mb-6 flex items-start md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold">🗑️ Thùng rác — Menu</h1>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/admin/menu" className="inline-flex">
              <Button variant="secondary" aria-label="Quay về danh sách menu">
                <ArrowLeft size={16} />
                Quay về menu
              </Button>
            </Link>
