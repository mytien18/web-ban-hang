...
const API  = BASE + "/api/v1/contacts";
...
export default function ContactsTrashPage() {
  ...
  const purgeRow = async (row) => {
    if (!confirm(`Xoá vĩnh viễn liên hệ "${row.name}"? Hành động này không thể hoàn tác.`)) return;
    try {
  ...
  const bulkPurge = async () => {
    if (selectedCount === 0) return;
    if (!confirm(`Xoá vĩnh viễn ${selectedCount} liên hệ đã chọn? Hành động không thể hoàn tác.`)) return;
    setBulkLoading(true);
    try {
  ...
  return (
    <>
      <Head>
        <title>Thùng rác — Liên hệ | Admin</title>
      ...
      <main id="main-content" className="p-6 anim-fade-in" aria-busy={loading ? "true" : "false"}>
        <nav aria-label="Bạn đang ở đây" className="text-sm text-gray-500 mb-1">
          <ol className="flex items-center gap-1">
            ...
            <li><Link href="/admin/contacts" className="hover:underline">Liên hệ</Link></li>
            <li aria-hidden="true">/</li>
        ...
        <header className="mb-6 flex items-start md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold">🗑️ Thùng rác — Liên hệ</h1>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/admin/contacts" className="inline-flex">
              <Button variant="secondary" aria-label="Quay về danh sách liên hệ">
                <ArrowLeft size={16} />
                Quay về liên hệ
              </Button>
            </Link>
                ...
                <tbody className="divide-y" aria-live="polite">
                  ...
                  {!loading && rows.map((row) => (
                    <tr key={row.id} className="hover:bg-gray-50/60 transition">
                      ...
                      <td className="px-4 py-3 text-gray-700 text-xs line-clamp-2">{row.content || "—"}</td>
