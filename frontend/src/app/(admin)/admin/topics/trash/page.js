...
export default function TopicsTrashPage() {
  ...
  const purgeRow = async (row) => {
    if (!confirm(`Xoá vĩnh viễn chủ đề "${row.name}"? Hành động này không thể hoàn tác.`)) return;
    try {
  ...
  const bulkPurge = async () => {
    if (selectedCount === 0) return;
    if (!confirm(`Xoá vĩnh viễn ${selectedCount} chủ đề đã chọn? Hành động không thể hoàn tác.`)) return;
    setBulkLoading(true);
    try {
  ...
  return (
    <>
      <Head>
        <title>Thùng rác — Chủ đề | Admin</title>
      ...
      <main id="main-content" className="p-6 anim-fade-in" aria-busy={loading ? "true" : "false"}>
        <nav aria-label="Bạn đang ở đây" className="text-sm text-gray-500 mb-1">
          <ol className="flex items-center gap-1">
            ...
            <li><Link href="/admin/topics" className="hover:underline">Chủ đề</Link></li>
            <li aria-hidden="true">/</li>
        ...
        <header className="mb-6 flex items-start md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold">🗑️ Thùng rác — Chủ đề</h1>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/admin/topics" className="inline-flex">
              <Button variant="secondary" aria-label="Quay về danh sách chủ đề">
                <ArrowLeft size={16} />
                Quay về chủ đề
              </Button>
            </Link>
                ...
                <tbody className="divide-y" aria-live="polite">
                  {loading && (
                    <tr className="animate-pulse">
                      <td className="px-4 py-3" colSpan={6}><div className="h-4 w-full bg-gray-100 rounded" /></td>
                    </tr>
                  )}
                  {!loading && rows.length === 0 && (
                    <tr><td colSpan={6} className="px-4 py-10 text-center text-gray-500">Thùng rác trống</td></tr>
                  )}
                  {!loading && rows.map((row) => (
                    <tr key={row.id} className="hover:bg-gray-50/60 transition">
                      ...
                      <td className="px-4 py-3 text-gray-700 font-mono text-xs">{row.slug}</td>
                      <td className="px-4 py-3 text-gray-700 text-xs">{row.description || "—"}</td>
