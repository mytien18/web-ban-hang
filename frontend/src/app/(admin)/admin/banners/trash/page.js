...
export default function BannersTrashPage() {
  ...
  const purgeRow = async (row) => {
    if (!confirm(`Xoá vĩnh viễn banner "${row.name}"? Hành động này không thể hoàn tác.`)) return;
    try {
  ...
  const bulkPurge = async () => {
    if (selectedCount === 0) return;
    if (!confirm(`Xoá vĩnh viễn ${selectedCount} banner đã chọn? Hành động không thể hoàn tác.`)) return;
    setBulkLoading(true);
    try {
  ...
  return (
    <>
      <Head>
        <title>Thùng rác — Banners | Admin</title>
      ...
      <main id="main-content" className="p-6 anim-fade-in" aria-busy={loading ? "true" : "false"}>
        <nav aria-label="Bạn đang ở đây" className="text-sm text-gray-500 mb-1">
          <ol className="flex items-center gap-1">
            ...
            <li><Link href="/admin/banners" className="hover:underline">Banners</Link></li>
            <li aria-hidden="true">/</li>
        ...
        <header className="mb-6 flex items-start md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold">🗑️ Thùng rác — Banners</h1>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/admin/banners" className="inline-flex">
              <Button variant="secondary" aria-label="Quay về danh sách banners">
                <ArrowLeft size={16} />
                Quay về banners
              </Button>
            </Link>
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50 text-left sticky top-0 z-10">
                  <tr>
                    ...
                    <th className="px-4 py-3 font-medium text-gray-600">Hình ảnh</th>
                    <th className="px-4 py-3 font-medium text-gray-600">Link</th>
                    <th className="px-4 py-3 font-medium text-gray-600">Vị trí</th>
                ...
                <tbody className="divide-y" aria-live="polite">
                  {loading && (
                    <tr className="animate-pulse">
                      <td className="px-4 py-3" colSpan={7}><div className="h-4 w-full bg-gray-100 rounded" /></td>
                    </tr>
                  )}
                  ...
                  {!loading && rows.map((row) => (
                    <tr key={row.id} className="hover:bg-gray-50/60 transition">
                      ...
                      <td className="px-4 py-3 text-gray-700 text-xs">{row.link || "—"}</td>
